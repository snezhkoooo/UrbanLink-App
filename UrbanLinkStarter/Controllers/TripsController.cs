...
public class TripsController : Controller
{
    ...
    public async Task<IActionResult> Index(string? q)
    {
        ...
        if (!string.IsNullOrWhiteSpace(q))
        {
            ...
            query = query.Where(t =>
                ...
                t.FromLocation.ToLower().Contains(needle) ||
                t.ToLocation.ToLower().Contains(needle) ||
                (t.Driver != null && t.Driver.FullName.ToLower().Contains(needle)));
        }

        var trips = await query.OrderBy(t => t.TravelDate).ToListAsync();
        ViewBag.Search = q;
        return View(trips);
    }
    ...
    public async Task<IActionResult> Create(int? eventId = null)
    {
        ...
        {
            TempData["Error"] = "Only verified drivers can create trips. Please complete driver verification first.";
            return RedirectToAction("Index", "DriverVerification");
        }
